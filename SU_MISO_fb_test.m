...
R_CSIT_ZF_mean = mean(R_CSIT_ZF_all, 2);

figure, plot(SNR_vector, real(R_CSIT_ZF_mean),'linewidth', 1)
set(gca, 'fontsize',14)
xlabel('SNR (dB)')
ylabel('Rate (bps/Hz)')
grid on;
...
R_fb_ZF_mean = mean(R_fb_ZF_all, 2);

hold on;
plot(SNR_vector, real(R_fb_ZF_mean),'--o','linewidth', 1)
legend1 = legend('Perfect CSIT','Limited Feedback');
set(legend1,'fontsize',14,'interpreter','latex','Location','best')
